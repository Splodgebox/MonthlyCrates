...
public class AnimationManager {
    ...
    public AnimationManager(Player player, String crate, MonthlyCrates plugin) {
        this.player = player;
        this.crate = crate;
        this.plugin = plugin;
    }
    ...
    public void openGUI() {
        ...
        slots.forEach(integer ->
                inventory.setItem(integer, new ItemStackBuilder(XMaterial.ENDER_CHEST.parseMaterial())
                        ...
                        .build(), (player, inventoryClickEvent) -> shuffleRewards(inventoryClickEvent.getSlot())
                ));
            ...
            public void run() {
                try {
                    ...
                    if (integer.get() == 10) {
                        ...
                        if (getCompletedRewards() == 9) setCompleted(true);
    ...
    public void finalReward() {
        ...
        setPanes(10, "GRAY_STAINED_GLASS_PANE");
        RandomCollection<RewardManager> randomCollection = new RandomCollection<>();
        for (String string : Objects.requireNonNull(plugin.crates.getConfiguration().getConfigurationSection("Crates." + crate + ".rewards.bonus-rewards")).getKeys(false)) {
            randomCollection.add(
                    plugin.crates.getConfiguration().getDouble("Crates." + crate + ".rewards.bonus-rewards." + string + ".chance"),
                    new RewardManager(
                            plugin.crates.getConfiguration().getDouble("Crates." + crate + ".rewards.bonus-rewards." + string + ".chance"),
                            Material.valueOf(plugin.crates.getConfiguration().getString("Crates." + crate + ".rewards.bonus-rewards." + string + ".material")),
                            plugin.crates.getConfiguration().getString("Crates." + crate + ".rewards.bonus-rewards." + string + ".name"),
                            plugin.crates.getConfiguration().getInt("Crates." + crate + ".rewards.bonus-rewards." + string + ".amount"),
                            plugin.crates.getConfiguration().getString("Crates." + crate + ".rewards.bonus-rewards." + string + ".command"),
                            plugin.crates.getConfiguration().getBoolean("Crates." + crate + ".rewards.bonus-rewards." + string + ".give_item")
                    ));
        }
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                for (int i = 0; i < inventory.getInventory().getSize(); i++) {
                    for (Integer integers : animationSlots.get(i)) {
                        inventory.setItem(
                                integers,
